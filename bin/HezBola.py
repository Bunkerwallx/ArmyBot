...
class WebCrawler:
    ...
    def start_crawling(self):
        ...
        # Preguntar por la URL base
        if not self.base_url:
            self.base_url = input("Enter base URL (e.g., https://example.com): ").strip()
            if not uparse(self.base_url).scheme:
                self.base_url = "https://" + self.base_url
        if not self.base_url.startswith("http"):
            print("Invalid URL format. Exiting.")
            return

        # Preguntar por el modo de inicio
        mode = input("Start in manual mode? (y/n): ").strip().lower()
        if mode == 'y':
            self.current_ip = input("Enter starting IP (e.g., 10.0.0.1): ").strip()
            self.ip_range = input("Enter IP range (CIDR): ").strip()
        else:
            # Rango de IPs predeterminado para modo automático
            self.current_ip = "10.0.0.1"
            self.ip_range = "10.0.0.0/24"
